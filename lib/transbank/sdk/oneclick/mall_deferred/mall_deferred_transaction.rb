  ...
  module Webpay
    module Oneclick
      class MallDeferredTransaction < Transbank::Webpay::Oneclick::MallTransaction
        ...
        TRANSACTION_CAPTURE_ENDPOINT = 'rswebpaytransaction/api/oneclick/v1.0/transactions/:token/capture'.freeze

        class << self

          def capture(token:, child_commerce_code:, child_buy_order:, authorization_code:, amount:, options: nil)
